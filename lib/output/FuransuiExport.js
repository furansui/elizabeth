...
FuransuiExport.prototype.exportDay = function exportDay(day, cb) {
	...
	day.segments.forEach(function(segment) {
		...
		if(segment.type == 'place') {
			text += start.format('YYYY.MM.DD HH:mm Z') + ',at ' + segment.place.name + ' for ' + end.diff(start, 'minutes') +  'min.\n';
			return;
		}
		...
		if(segment.type == 'move' && Array.isArray(segment.activities)) {
			segment.activities.forEach(function(activity) {
				...
				text += start.format('YYYY.MM.DD HH:mm Z') + ',' + activity.activity + ' for ' + activity.duration + ' seconds (' + activity.distance + 'm)\n';
			    //}
			});
		}
	});
